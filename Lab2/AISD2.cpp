...
int gallop(int arr[], int key, int left, int right, bool findRightBound = false) {
    if (findRightBound) {
        int last = right;
        int offset = 1;
...
void mergeWithGallop(int arr[], int left, int mid, int right) {
    ...
    while (i < len1 && j < len2) {
        ...
        if (gallopCount >= 7) {
            ...
            if (i < len1 && j < len2) {
                if (L[i] <= R[j]) {
                    int gallopEnd = gallop(L, R[j], i, len1 - 1, false);
                    while (i < gallopEnd) {
                        arr[k++] = L[i++];
                    }
                }
                else {
                    int gallopEnd = gallop(R, L[i], j, len2 - 1, true);
                    while (j < gallopEnd) {
                        arr[k++] = R[j++];
                    }
                }
