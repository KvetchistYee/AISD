...
int findRunLength(int arr[], int start, int n) {
    ...
    if (!ascending) {
        reverse(arr + start, arr + start + length + 1);
    }
...
void mergeWithGallop(int arr[], int l, int m, int r) {
    ...
    while (i < len1 && j < len2) {
        ...
        if (left_count >= gallop_threshold) {
            int pos = binarySearch(right.data(), j, len2, left[i]);
            while (j < pos) {
                arr[k++] = right[j++];
            }
        } else if (right_count >= gallop_threshold) {
            int pos = binarySearch(left.data(), i, len1, right[j]);
            while (i < pos) {
                arr[k++] = left[i++];
            }
        }
...
void timSort(int arr[], int n) {
    ...
    while (i < n) {
        ...
        while (runs.size() >= 2) {
            Run r = runs.top(); runs.pop();
            Run l = runs.top(); runs.pop();
            ...
            runs.push(Run(l.start, l.length + r.length));
        }
